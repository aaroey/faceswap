...
class ConvertImage(DirectoryProcessor):
  ...
  def add_optional_arguments(self, parser):
    ...
    parser.add_argument(
        ...
        choices=(
            "Masked",
            "Adjust"),  # case sensitive because this is used to load a plugin.
        default="Masked",
  ...
  def process(self):
    ...
    model = PluginLoader.get_model(model_name)(
        get_folder(self.arguments.model_dir))
  ...
  def check_skipface(self, filename, face_idx):
    aligned_face_name = '{}_{}{}'.format(
        Path(filename).stem, face_idx,
        Path(filename).suffix)
    aligned_face_file = Path(
        self.arguments.input_aligned_dir) / Path(aligned_face_name)
    # TODO: Remove this temporary fix for backwards compatibility of filenames
    bk_compat_aligned_face_name = '{}{}{}'.format(
        Path(filename).stem, face_idx,
        Path(filename).suffix)
    bk_compat_aligned_face_file = Path(
        self.arguments.input_aligned_dir) / Path(bk_compat_aligned_face_name)
        ...
        for idx, face in faces:
          ...
          if face.r != 0:
            ...
            image = converter.patch_image(
                image, face, 64 if "128" not in self.arguments.trainer else 128)
    ...
    for filename in tqdm(self.read_directory()):
      ...
      if is_have_alignments:
        if self.have_face(filename):
          ...
          print('no alignment found for {}, skipping'.format(
