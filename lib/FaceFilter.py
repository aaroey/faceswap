...
class FaceFilter():
  ...
  def __init__(self,
    ...
    self.encodings = list(
        map(lambda im: face_recognition.face_encodings(im)[0], images))
    self.nencodings = list(
        map(lambda im: face_recognition.face_encodings(im)[0], nimages))
  ...
  def check(self, detected_face):
    ...
    if encodings is not None and len(encodings) > 0:
      distances = list(
          face_recognition.face_distance(self.encodings, encodings[0]))
      ...
      if len(self.nencodings) > 0:
        ndistances = list(
            face_recognition.face_distance(self.nencodings, encodings[0]))
