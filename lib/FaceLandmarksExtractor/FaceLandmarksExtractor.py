...
class TorchBatchNorm2D(keras.engine.base_layer.Layer):
  ...
  def build(self, input_shape):
    ...
    self.moving_mean = self.add_weight(
        shape=shape, name='moving_mean', initializer='zeros', trainable=False)
  ...
  def call(self, inputs, training=None):
    ...
    invstd = K.ones(
        ...
        dtype='float32') / K.sqrt(broadcast_moving_variance +
                                  K.constant(self.epsilon, dtype='float32'))

    return (inputs -
            broadcast_moving_mean) * invstd * broadcast_gamma + broadcast_beta
...
def crop(image, center, scale, resolution=256.0):
  ...
  br = transform([resolution, resolution], center, scale,
...
def extract(input_image,
  ...
  input_image = cv2.resize(
      input_image, (int(w * input_scale), int(h * input_scale)),
      interpolation=cv2.INTER_LINEAR)
  input_image_bgr = input_image[:, :, ::-1].copy(
  )  #cv2 and numpy inputs differs in rgb-bgr order, this affects chance of dlib face detection
  input_images = [input_image, input_image_bgr]

  detected_faces = []
  if use_cnn_face_detector:
    for detector, input_image in ((detector, input_image)
                                  for detector in dlib_detectors
  ...
  if len(detected_faces) > 0:
    for i, d_rect in enumerate(detected_faces):
      ...
      left, top, right, bottom = d_rect.left(), d_rect.top(), d_rect.right(
      ), d_rect.bottom()
      del d_rect
      ...
      pts_img = get_pts_from_predict(
          keras_model.predict(image)[-1][0], center, scale)
      pts_img = [
          (int(pt[0] / input_scale), int(pt[1] / input_scale)) for pt in pts_img
      ]
      landmarks.append(
          ((int(left / input_scale), int(top / input_scale),
            int(right / input_scale), int(bottom / input_scale)), pts_img))
  else:
