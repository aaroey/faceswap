...
class TrainingDataGenerator():
  ...
  def __init__(self,
               ...
               coverage,
               scale=5,
               zoom=1):  #TODO thos default should stay in the warp function
  ...
  def read_image(self, fn):
    ...
    warped_img, target_img = self.random_warp(image, self.coverage, self.scale,
                                              self.zoom)

    return warped_img, target_img

  def random_transform(self, image, rotation_range, zoom_range, shift_range,
                       random_flip):
    h, w = image.shape[0:2]
  ...
  def random_warp(self, image, coverage, scale=5, zoom=1):
    ...
    interp_mapx = cv2.resize(
        mapx, (80 * zoom, 80 * zoom))[8 * zoom:72 * zoom,
                                      8 * zoom:72 * zoom].astype('float32')
    interp_mapy = cv2.resize(
        mapy, (80 * zoom, 80 * zoom))[8 * zoom:72 * zoom,
                                      8 * zoom:72 * zoom].astype('float32')

    warped_image = cv2.remap(image, interp_mapx, interp_mapy, cv2.INTER_LINEAR)

    src_points = numpy.stack([mapx.ravel(), mapy.ravel()], axis=-1)
    dst_points = numpy.mgrid[0:65 * zoom:16 * zoom,
                             0:65 * zoom:16 * zoom].T.reshape(-1, 2)
...
def stack_images(images):
  ...
  return numpy.transpose(
      images, axes=numpy.concatenate(new_axes)).reshape(new_shape)
