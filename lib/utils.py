...
def get_image_paths(directory, exclude=[], debug=False):
  exclude_names = [
      basename(Path(x).stem[:-1] + Path(x).suffix) for x in exclude
  ]
  dir_contents = []
...
def rotate_image(image, angle):
  ...
  if angle == 90:
    ...
    print(
        'Unsupported image rotation angle: {}. Image unmodified'.format(angle))
  return image
...
class BackgroundGenerator(threading.Thread):
  ...
  def __init__(self,
               generator,
               prefetch=1):  #See below why prefetch count is flawed
