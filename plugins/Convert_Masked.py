...
class Convert():
  ...
  def __init__(self,
    ...
    if erosion_kernel_size is not None:
      if erosion_kernel_size > 0:
        ...
        self.erosion_kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE,
            (abs(erosion_kernel_size), abs(erosion_kernel_size)))
  ...
  def patch_image(self, image, face_detected, size):
    ...
    image_mask = self.get_image_mask(image, new_face, face_detected, mat,
                                     image_size)

    return self.apply_new_face(image, new_face, image_mask, mat, image_size,
                               size)
    ...
    if self.seamless_clone:
      ...
      if maxregion.size > 0:
        ...
        outimage = cv2.seamlessClone(
            new_image.astype(numpy.uint8), base_image.astype(numpy.uint8),
  ...
  def hist_match(self, source, template, mask=None):
    ...
    s_values, bin_idx, s_counts = numpy.unique(
        source, return_inverse=True, return_counts=True)
    t_values, t_counts = numpy.unique(template, return_counts=True)
    ms_values, mbin_idx, ms_counts = numpy.unique(
        source, return_inverse=True, return_counts=True)
  ...
  def color_hist_match(self, src_im, tar_im, mask):
    ...
    matched = numpy.stack((matched_R, matched_G, matched_B),
                          axis=2).astype(src_im.dtype)
    return matched
  ...
  def get_new_face(self, image, mat, size):
    ...
    if "GAN" not in self.trainer:
      ...
      new_face = numpy.clip((new_face[0] + 1) * 255 / 2, 0,
  ...
  def get_image_mask(self, image, new_face, face_detected, mat, image_size):
    ...
    if 'hull' in self.mask_type:
      hull = cv2.convexHull(
          numpy.array(face_detected.landmarksAsXY()).reshape(
