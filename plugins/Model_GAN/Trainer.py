...
class Trainer():
  ...
  def setup(self):
    distorted_A, fake_A, mask_A, self.path_A, self.path_mask_A, self.path_abgr_A, self.path_bgr_A = self.cycle_variables(
        self.model.netGA)
    distorted_B, fake_B, mask_B, self.path_B, self.path_mask_B, self.path_abgr_B, self.path_bgr_B = self.cycle_variables(
        self.model.netGB)
    ...
    if self.use_lsgan:
      ...
      self.loss_fn = lambda output, target: -K.mean(
          ...
          (1 - target))
    self.loss_fn_bce = lambda output, target: -K.mean(
        K.log(output + 1e-12) * target + K.log(1 - output + 1e-12) *
        (1 - target))
    ...
    if self.use_perceptual_loss:
      ...
      vggface = VGGFace(
          include_top=False, model='resnet50', input_shape=(224, 224, 3))
    ...
    loss_DA, loss_DA2, loss_GA, loss_DA_feat, loss_DA_code = self.define_loss(
        self.model.netDA, self.model.netDA2, netDA_feat, self.model.netD_code,
        self.model.netGA, real_A, fake_A, distorted_A, "A", vggface_feat)
    loss_DB, loss_DB2, loss_GB, loss_DB_feat, loss_DB_code = self.define_loss(
        self.model.netDB, self.model.netDB2, netDB_feat, self.model.netD_code,
        self.model.netGB, real_B, fake_B, distorted_B, "B", vggface_feat)
    ...
    training_updates = Adam(
        lr=self.lrD, beta_1=0.5).get_updates(weightsDA, [], loss_DA)
    self.netDA_train = K.function([distorted_A, real_A], [loss_DA],
                                  training_updates)
    training_updates = Adam(
        lr=self.lrD, beta_1=0.5).get_updates(weightsDA2, [], loss_DA2)
    self.netDA2_train = K.function([distorted_A, real_A], [loss_DA2],
                                   training_updates)
    training_updates = Adam(
        lr=self.lrG, beta_1=0.5).get_updates(weightsGA, [], loss_GA)
    self.netGA_train = K.function([distorted_A, real_A], [loss_GA],
                                  training_updates)

    training_updates = Adam(
        lr=self.lrD, beta_1=0.5).get_updates(weightsDB, [], loss_DB)
    self.netDB_train = K.function([distorted_B, real_B], [loss_DB],
                                  training_updates)
    training_updates = Adam(
        lr=self.lrD, beta_1=0.5).get_updates(weightsDB2, [], loss_DB2)
    self.netDB2_train = K.function([distorted_B, real_B], [loss_DB2],
                                   training_updates)
    training_updates = Adam(
        lr=self.lrG, beta_1=0.5).get_updates(weightsGB, [], loss_GB)
    self.netGB_train = K.function([distorted_B, real_B], [loss_GB],
                                  training_updates)

    training_updates = Adam(
        lr=self.lrD, beta_1=0.5).get_updates(weightsD_code, [], loss_DA_code)
    self.netDA_code_train = K.function([distorted_A, real_A], [loss_DA_code],
                                       training_updates)
    training_updates = Adam(
        lr=self.lrD, beta_1=0.5).get_updates(weightsD_code, [], loss_DB_code)
    self.netDB_code_train = K.function([distorted_B, real_B], [loss_DB_code],
                                       training_updates)
  ...
  def first_order(self, x, axis=1):
    ...
    img_ncols = x.shape[2]
    if axis == 1:
      return K.abs(x[:, :img_nrows - 1, :img_ncols - 1, :] -
                   x[:, 1:, :img_ncols - 1, :])
    elif axis == 2:
      return K.abs(x[:, :img_nrows - 1, :img_ncols - 1, :] -
                   x[:, :img_nrows - 1, 1:, :])
  ...
  def train_one_step(self, iter, viewer, save_interval):
    ...
    if (int(iter / save_interval) % 15 == 0 and iter % save_interval == 0 and
        int(iter / save_interval) > 0) or iter == 0:
      print(
          'Losses:                     {}DA,       DB,       DA2,      DB2,      DA_code,  DB_code,  GA,       GB'
          .format(' ' * len(str(iter))))

    print(
        '[%s] [%d/%s][%d] %f, %f, %f, %f, %f, %f, %f, %f' %
        (time.strftime("%H:%M:%S"), epoch, "num_epochs", iter, errDA_avg,
         errDB_avg, errDA2_avg, errDB2_avg, errDA_code_avg, errDB_code_avg,
         errGA_avg, errGB_avg),
        end='\r')
  ...
  def show_sample(self, display_fn):
    ...
    display_fn(
        self.showG_mask(tA, tB, self.path_mask_A, self.path_mask_B), "mask")
  ...
  def showG(self, test_A, test_B, path_A, path_B):
    figure_A = np.stack([
        ...
        np.squeeze(
            np.array(
                [path_A([test_A[i:i + 1]]) for i in range(test_A.shape[0])])),
        np.squeeze(
            np.array(
                [path_B([test_A[i:i + 1]]) for i in range(test_A.shape[0])])),
    ],
                        axis=1)
    figure_B = np.stack([
        test_B,
        np.squeeze(
            np.array(
                [path_B([test_B[i:i + 1]]) for i in range(test_B.shape[0])])),
        np.squeeze(
            np.array(
                [path_A([test_B[i:i + 1]]) for i in range(test_B.shape[0])])),
    ],
                        axis=1)
