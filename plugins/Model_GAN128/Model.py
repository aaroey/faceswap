...
class GANModel():
  ...
  def build_generator(self):
    ...
    def mixed_scaled_dense_network(input_tensor,
                                   num_layers=32,
                                   nc_in=3,
                                   nc_out=1):
      """
      ...
      for i in range(num_layers):
        dilation = (i % 10) + 1
        msd_layers["layer{0}".format(i)] = Conv2D(
            1,
            kernel_size=3,
            strides=1,
        ...
        for j in range(i):
          ...
          conv_3x3 = Conv2D(
              ...
              use_bias=True,
              padding="same")(
                  msd_layers["layer{0}".format(j)])
          msd_layers["layer{0}".format(i)] = add(
              [msd_layers["layer{0}".format(i)], conv_3x3])
        msd_layers["layer{0}".format(i)] = Activation("relu")(
            msd_layers["layer{0}".format(i)])
      ...
      out = Conv2D(
          nc_out, kernel_size=1, kernel_initializer=conv_init, padding="same")(
              concat_all)
