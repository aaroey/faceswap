...
class Convert(object):
  ...
  def patch_image(self, original, face_detected, size):
    ...
    original[slice(face_detected.y, face_detected.y + face_detected.h),
             slice(face_detected.x, face_detected.x +
                   face_detected.w)] = cv2.resize(
                       new_face, (face_detected.w, face_detected.h))
    return original
